<template>
	<div class="home-preview">
		...
		<!-- 新闻资讯 -->
		<!-- 商品推荐 -->
		<div id="animate_recommendstzc" class="recommend animate__animated">
			<div class="recommend_title_box">
				<span class="recommend_title">电气设备推荐</span>
				<span class="recommend_subhead">{{'stzc'.toUpperCase()}} RECOMMEND</span>
			</div>
			<div v-if="stzcRecommend.length" class="list list12 index-pv1 list12stzc">
				<div class="list-body">
					<div class="item list-item animation-box" v-for="item,index in stzcRecommend" :key="index" @click="toDetail('stzcDetail', item)">
						<img :name="item.id" v-if="preHttp(item.imgurl)&&preHttp2(item.imgurl)" :src="item.imgurl" alt="" />
						<img :name="item.id" v-else-if="preHttp(item.imgurl)" :src="item.imgurl.split(',')[0]" alt="" />
						<img :name="item.id" v-else :src="baseUrl + (item.imgurl?item.imgurl.split(',')[0]:'')" alt="" />
						<div class="name">{{item.title}}</div>
					</div>
				</div>
			</div>
			<div class="moreBtn" @click="moreBtn('stzc')">
				<span class="text">查看更多</span>
				<i class="icon iconfont icon-gengduo1"></i>
			</div>
		</div>
		...
		methods: {
			...
			handleScroll() {
				let arr = [
					...
					{id:'animate_recommendstzc',css:'animate__backInRight'},
			...
			getList() {
				let autoSortUrl = "";
				let data = {}
				autoSortUrl = "stzc/autoSort";
				if(localStorage.getItem('frontToken')) {
					autoSortUrl = "stzc/autoSort2";
				}
				data = {
					page: 1,
					limit: 8,
				...
				this.$http.get(autoSortUrl, {params: data}).then(res => {
					if (res.data.code == 0) {
						this.stzcRecommend = res.data.data.list;
					}
				});
