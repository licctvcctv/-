...
///////////////////////////////////////////////以下为新版本协同推荐算法逻辑///////////////////////////////////////////////////////
    ...
    public List<String> recommendItems(List entities, String x, String y, String xValue, int numRecommendations) {
        ...
        try {
            ...
            logger.info("成功为用户 {} 推荐了 {} 个物品: {}", xValue, recommendations.size(), recommendations);
        } catch (Exception e) {
    ...
    private Map<String, Double> calculateSimilarityScores(Map<String, Map<String, Double>> xMatrix, String currentUserid) {
        ...
        logger.info("开始计算用户 {} 的相似度得分", currentUserid);
            ...
            for (Map.Entry<String, Map<String, Double>> entry : xMatrix.entrySet()) {
                ...
                if (!otherUserid.equals(currentUserid)) {
                    ...
                    if (similarity > 0) {
                        ...
                        logger.info("用户 {} 和用户 {} 的相似度为 {}", currentUserid, otherUserid, similarity);
                    }
                }
            }
        }
        logger.info("用户 {} 的相似度得分计算完成，共找到 {} 个相似用户", currentUserid, similarityScores.size());
